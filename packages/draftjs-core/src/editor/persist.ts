...
export const persistState = (editorState: EditorState): PersistedState => {
  ...
  Object.entries(catalog).forEach(([n,k]) => key2name[k] = n);
...
export const createContentState = (
  ...
  const cs3 = Object.entries(ctnt.entities).reduce((csi, el) => {
    const [key, ed] = el;
    let cso = csi.createEntity(ed.type, ed.mutability as DraftEntityMutability, ed.data);
  ...
  let cache = new Map<CharacterMetadata,CharacterMetadata>();
