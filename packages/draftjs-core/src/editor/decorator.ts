...
export default (editorOpts: EditorOpts, decorators: DecoratorsT = DECORATORS): DraftDecoratorType  => {
  const self = {} as DraftDecoratorType;

  let id2entities = {} as {[key:string]:Immutable.Set<[string, Entity]>}
  let mappers = {} as {[key:string]:DecorationMapper};
  let styles = {} as {[key:string]:DecoratorClassDef};
  ...
  self.getDecorations = (block: ContentBlock, contentState: ContentState): Immutable.List<string> =>  {
    return block.getCharacterList().map((cm?) => {
      if (!cm) return null;

      const sy = cm.getStyle();
      if (sy.size === 0) return null;

      const d = createDecorator(sy, contentState);
      return d;
    }) as Immutable.List<string>;
  }

  const createDecorator = (styles: Immutable.OrderedSet<string>, contentState: ContentState): string => {
    const sa = styles.toArray();
    const id = sa.sort().join('.');

    const sm = styles.map(k => {
